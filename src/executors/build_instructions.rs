...
use solana_farm_client::raydium_sdk::{
    make_swap_fixed_in_instruction, LiquiditySwapFixedInInstructionParamsV4, UserKeys,
};
...
pub async fn build_instructions(
    context: &AppContext,
    action: &Arc<Mutex<SolanaAction>>,
    price_per_cu_microlamports: u64,
    compute_units_per_tx: u32,
) -> Result<(Balance, Vec<Instruction>)> {
    ...
    let sol_balance = solana::get_balance(context, &action_guard.sniper.pubkey())
        .await
        .unwrap_or(0);
    if sol_balance == 0 {
    ...
    let token_balance = solana::get_token_balance(context, &sniper_pubkey, &token_mint)
        .await
        .unwrap_or(0);
    let sniper_token_ata =
        spl_associated_token_account::get_associated_token_address(&sniper_pubkey, &token_mint);
    let sniper_wsol_ata = spl_associated_token_account::get_associated_token_address(
        &sniper_pubkey,
        &WSOL_MINT_PUBKEY,
    );
    ...
    // 2.b. swap TOKEN:
    let tx_fee = BASE_TX_FEE_SOL
        + (price_per_cu_microlamports * compute_units_per_tx as u64) / 1_000_000
        + 1;
        ...
        if let Some((step_ixs, sol_cost, token_cost, min_sol_to_keep_after)) = match step {
                ...
                match transfer.asset {
                    Asset::Sol => {
                        match transfer.amount {
                            ...
                            Amount::ExactWithFees(amount) => {
                                if amount > 0 {
                                    ...
                                    Some((
                                        ...
                                        amt_to_transfer + tx_fee_pointer,
                                        0,
                                        0,
                                    ))
                                } else {
                            ...
                            Amount::Max | Amount::MaxAndClose => Some((
                                ...
                                sol_balance_pointer - tx_fee_pointer,
                                0,
                                0,
                            )),
                            Amount::MaxButLeaveForTransfer => {
                                if sol_balance_pointer > tx_fee + tx_fee_pointer {
                                    let amt_to_transfer =
                                        sol_balance_pointer - tx_fee - tx_fee_pointer;
                                    Some((
                                        vec![solana_sdk::system_instruction::transfer(
                                            &sniper_pubkey,
                                            &transfer.receiver,
                    ...
                    Asset::Token(token_pubkey) => {
                        ...
                        let mut token_amt = match transfer.amount {
                            ...
                            Amount::MaxButLeaveForTransfer | Amount::Max | Amount::MaxAndClose => {
                                token_balance_pointer
                            }
                        };
                        if token_amt > 0 {
                            ...
                            if !solana::is_account_exist(context, &receiver_ata).await {
                                debug!(
                                    "Creating ATA for the receiver, fee payer: {:?}, ata: {:?}",
                                    fee_payer, receiver_ata
                                );
            ...
            SolanaActionPayload::SolanaSwapActionPayload(swap) => {
                if !(swap.keys.base_mint != *WSOL_MINT_PUBKEY
                    && swap.keys.quote_mint == *WSOL_MINT_PUBKEY)
                {
                    bail!(ExecutionError::UnsupportedPool(
                        swap.keys.base_mint.to_string(),
                        swap.keys.quote_mint.to_string()
                    ));
                }
                // swapping
                let user_keys = match swap.swap_method {
                    SwapMethod::BuyTokensForExactSol => {
                        // reversed pool
                        if swap.keys.base_mint == *WSOL_MINT_PUBKEY {
                            UserKeys::new(sniper_token_ata, sniper_wsol_ata, sniper_pubkey)
                        } else {
                            UserKeys::new(sniper_wsol_ata, sniper_token_ata, sniper_pubkey)
                        }
                    }
                ...
                match swap.swap_method {
                    SwapMethod::BuyTokensForExactSol => {
                        ...
                        let wsol_ata_creation_fee_reimbursed =
                        ...
                        debug!(
                            "wsol_ata_creation_fee_reimbursed: {}",
                            wsol_ata_creation_fee_reimbursed
                        );
                        let mut swap_sol_amount_in = match swap.amount_in {
                            ...
                            Amount::ExactWithFees(amount) => {
                                let fees = tx_fee_pointer
                                    + ata_creation_fee
                                    + wsol_ata_creation_fee_reimbursed;
                                if amount > sol_balance_pointer {
                                    bail!(ExecutionError::NotEnoughSolBalance(
                                        amount,
                                        sol_balance_pointer
                                    ));
                                }
                        ...
                        if swap_sol_amount_in > 0 {
                            ...
                            Some((
                                token_transfer_ixs,
                                swap_sol_amount_in + sol_to_budget,
                                0,
                                RENT_EXEMPTION_THRESHOLD_SOL,
                            ))
                        } else {
            ...
            if token_cost > token_balance_pointer {
                ...
                bail!(ExecutionError::NotEnoughTokenBalance(
                    token_cost + token_balance_pointer,
                    token_balance
                ));
            }
...
pub async fn estimate_cu_per_tx(context: &AppContext, action: &Arc<Mutex<SolanaAction>>) -> u32 {
    ...
    action
        ...
        .map(|s| match s {
            ...
            SolanaActionPayload::SolanaSwapActionPayload(solana_swap_action_payload) => {
                match solana_swap_action_payload.swap_method {
                    SwapMethod::BuyTokensForExactSol => {
                        ESTIMATE_CU_BUY_SPL_RAYDIUM_V4 + ESTIMATE_CU_SPL_TOKEN_TRANSFER
                    }
        ...
        .sum::<u32>()
        + ESTIMATE_CU_PER_COMPUTE_BUDGET_INSTRUCTION
}
