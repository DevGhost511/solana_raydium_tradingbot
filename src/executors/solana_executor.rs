...
use crate::config::constants::{
    ACTION_EXPIRY_S, BASE_TX_FEE_SOL, RENT_EXEMPTION_THRESHOLD_SOL, SIMULATION_RETRIES,
};
...
use crate::types::actions::{Amount, Asset, SolanaAction, SolanaActionPayload};
use crate::types::engine::{EventStream, Executor};
use crate::types::events::{
    BlockchainEvent, BotEvent, ExecutionError, ExecutionReceipt, ExecutionResult,
};
    ...
    async fn execute(&self, action: Arc<Mutex<SolanaAction>>) -> Result<BotEvent> {
        ...
        if action.lock().await.is_expired() {
            return Ok(BotEvent::ExecutionResult(
                ...
                action.clone(),
                ExecutionResult::ExecutionError(ExecutionError::ActionTooOld),
            ));
        }
        ...
        debug!(
            "Estimated compute units per tx: {:?}",
        ...
        .await
        {
            Ok((balance_before, mut action_itx)) => {
                ...
                if action_itx.is_empty() {
                    return Ok(BotEvent::ExecutionResult(
                        action.lock().await.uuid,
                        action.clone(),
                        ExecutionResult::ExecutionError(ExecutionError::NoInstructionsGenerated),
                    ));
                }
                ...
                let mut prefinal_itxs_with_cu = if price_per_cu_priority > 0 {
                    vec![
                ...
                let mut tx =
                    Transaction::new_with_payer(&prefinal_itxs_with_cu, Some(&fee_payer.pubkey()));
                ...
                action.lock().await.sent(
                    balance_before,
                    signature,
                    BASE_TX_FEE_SOL as i64
                        + price_per_cu_priority as i64 * compute_units_per_tx_estimate as i64,
                );
            ...
            Err(e) => {
                ...
                match e.downcast_ref::<ExecutionError>() {
                    Some(ExecutionError::Other(error_text)) => Ok(BotEvent::ExecutionResult(
                        action.lock().await.uuid,
                        Arc::clone(&action),
                        ExecutionResult::ExecutionError(ExecutionError::Other(error_text.clone())),
                    )),
                    _ => bail!(e),
                }
            }
