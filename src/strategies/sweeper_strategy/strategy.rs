...
#[derive(Clone)]
pub struct SweeperStrategy {
    pub state_machine: StateMachine<SweeperStrategyStateMachine>,
}

impl Debug for SweeperStrategy {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "SweeperStrategy, config: {:?}, state: {:?}",
...
impl SweeperStrategy {
    /// Create a new instance of the strategy.
    pub async fn new(
        context: &AppContext,
        strategy_config: &VolumeStrategyInstance,
    ) -> Result<Self> {
        let state_machine = SweeperStrategyStateMachine::new(context, strategy_config)
            .await?
            .state_machine();
...
#[async_trait]
impl Strategy<BotEvent, Arc<Mutex<SolanaAction>>> for SweeperStrategy {
    ...
    async fn process_event(&mut self, event: BotEvent) -> Vec<Arc<Mutex<SolanaAction>>> {
        ...
        let mut strat_actions_generated_from_event = self
            .state_machine
