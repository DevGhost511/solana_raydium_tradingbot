...
use crate::config::constants::{
    DELAY_BETWEEN_SIMULATION_RETRIES_MS, RETRIES_IF_ERROR_OR_TIMEOUT,
    TIMEOUT_FOR_ACTION_EXECUTION_HBS,
};
...
use crate::types::actions::{
    Amount, Asset, SolanaAction, SolanaActionPayload, SolanaSwapActionPayload,
...
impl SniperAgentState {
    ...
    pub async fn new(
        ...
        debug!(
            "Agent created for the pool: {:?}, token: {:?}, deployment price: {:.9} SOL",
            agent.pool, agent.pool.base_mint, agent.deploy_price.price
        );
    ...
    pub async fn queue_action(&mut self, action: SolanaAction) {
        ...
        self.strat_actions_generated_from_event
            .lock()
            .await
            .push(action);
    }
    ...
    pub(crate) async fn process_execution_receipt(
        ...
        if self.actions_in_progress.contains(&receipt.action_uuid) {
            self.actions_in_progress
                .retain(|&x| x != receipt.action_uuid);
            debug!(
                "Agent token `{:?}` received execution receipt: {:?}",
                self.pool.base_mint, receipt
            );
            match &receipt.err {
                Some(err) => {
    ...
    pub async fn get_sol_balance(&self) -> u64 {
        solana::get_balance(&self.context, &self.agent_key.pubkey())
            .await
            .unwrap_or(0)
    }

    pub async fn get_token_balance(&self) -> u64 {
        solana::get_token_balance(
            &self.context,
            &self.agent_key.pubkey(),
            &self.get_token_mint(),
        )
...
impl SniperAgentState {
    ...
    async fn set_buy_delay_timer(&mut self) {
        ...
        debug!(
            "Token `{:?}` setting buy delay timer {} ms",
            self.pool.base_mint, self.sniping_strategy_instance.buy_delay_ms
        );
    }
    ...
    async fn waiting_to_buy(&self, event: &SolanaStrategyEvent) -> Response<State> {
        ...
        debug!(
            "Token `{:?}` waiting to buy, elapsed: {} ms",
            self.pool.base_mint, elapsed_ms
        );
        if self.sniping_strategy_instance.buy_delay_ms == 0
            || (elapsed_ms > self.sniping_strategy_instance.buy_delay_ms as u128)
        {
        ...
        match event {
            SolanaStrategyEvent::Original(BotEvent::BlockchainEvent(
                BlockchainEvent::RaydiumHeartbeatPriceUpdate(price_update),
            ))
            | SolanaStrategyEvent::Original(BotEvent::BlockchainEvent(
                BlockchainEvent::RaydiumSwapEvent(RaydiumSwapEvent { price_update, .. }),
            )) => {
                if price_update.pool == self.pool.id {
                    let relative_price_drop_per_cent = (100.0
                        * (self.deploy_price.price - price_update.price))
                        / self.deploy_price.price;
                    debug!(
                        "Price change(buy): {:.5}% from deployment price",
                        -relative_price_drop_per_cent
                    );
                    if relative_price_drop_per_cent
                        > self.sniping_strategy_instance.skip_if_price_drops_percent
                    {
                        info!(
                            "Token `{:?}` price dropped by {:.5}%, skipping",
                            self.pool.base_mint, relative_price_drop_per_cent
                        );
    ...
    async fn buy(&mut self, amt: &Amount, retry: &i64) {
        if retry > &0 {
            tokio::time::sleep(tokio::time::Duration::from_millis(
                DELAY_BETWEEN_SIMULATION_RETRIES_MS,
            ))
            .await;
        }
        debug!(
            "Buying token `{:?}` for {:?} SOL, retry: {retry}",
            self.pool.base_mint,
            amt,
            retry = retry
        );
        self.queue_action(SolanaAction::new(
            self.agent_key.clone(),
            vec![SolanaActionPayload::SolanaSwapActionPayload(
    ...
    async fn buying(
        ...
        self.conditional_transition(
            event,
            State::buying(amt.clone(), retry + 1),
    ...
    async fn waiting_to_sell(&mut self, event: &SolanaStrategyEvent) -> Response<State> {
        ...
        match event {
            ...
            | SolanaStrategyEvent::Original(BotEvent::BlockchainEvent(
                ...
                if price_update.pool == self.pool.id {
                    let relative_price_change_per_cent = (100.0
                        * (price_update.price - self.buy_price.price))
                        / self.buy_price.price;
                    debug!(
                        "Token(sell) `{:?}` price change: {:.5}%, event: {:?}",
                        self.pool.base_mint, relative_price_change_per_cent, event
                    );
                    debug!(
                        "Price change(sell): {:.5}% from buy price",
                        relative_price_change_per_cent
                    );
                    if price_update.price
                        < self.buy_price.price
                            * self.sniping_strategy_instance.stop_loss_percent_move_down
                    {
                        info!(
                            "{:?}, selling the token at SL, {}",
                            self.pool.id, price_update.price
                        );
                        return Transition(State::selling(Amount::Max, 0));
                    } else if price_update.price
                        > self.buy_price.price
                            * self.sniping_strategy_instance.take_profit_percent_move_up
                    {
                        info!(
                            "{:?}, selling the token at TP, {}",
                            self.pool.id, price_update.price
                        );
                        return Transition(State::selling(Amount::Max, 0));
                    }
        ...
        if elapsed - self.last_time > 1 {
            info!(
                "Token `{:?}` waiting to sell, {:?} s left",
                self.pool.base_mint,
                (self.sniping_strategy_instance.force_exit_horizon_s - elapsed as i64).max(0)
            );
    ...
    async fn selling(
        ...
        self.conditional_transition(
            event,
            State::selling(Amount::Max, retry + 1),
            State::done(),
            &(retry + 1),
        )
        .await
    ...
    pub async fn error(&mut self, msg: &String, event: &SolanaStrategyEvent) -> Response<State> {
        error!(
            "Token `{:?}` error: {:?}, sell manually if token left",
            self.pool.base_mint, msg
        );
        Handled
    }
