...
impl AgentState {
    ...
    pub async fn new(
        ...
        if let Some(strategy_id) = strategy_id_opt {
            agent.trader = agent
                .db_read_or_create(&context.db_pool, &strategy_id)
                .await?;
    ...
    pub async fn new_from_trader(
        ...
        let _ =
            solana::get_token_balance(context, &agent.agent_key.pubkey(), &pool.base_mint).await;
        debug!(
            "Agent {} created from loaded trader record: {:?}",
            agent.trader.id,
            agent.agent_key.pubkey()
        );
        Ok(agent)
        ...
        match trader {
            ...
            Err(_) => {
                let new_trader = NewTrader {
                    ...
                    private_key: utils::keys::private_key_string_base58(
                        &self.agent_key.get_keypair(),
                    ),
    ...
    pub(crate) async fn process_execution_receipt(
        ...
        if self.actions_in_progress.contains(&receipt.action_uuid) {
            ...
            debug!(
                "Agent `{:?}` {} received execution receipt: {:?}",
                self.pubkey(),
    ...
    pub async fn get_main_wallet_pubkey(&self) -> Pubkey {
        self.main_wallet
            .clone()
            .unwrap_or(self.agent_key.clone())
            .pubkey()
    }
}
...
impl AgentState {
    ...
    async fn idle(&self, event: &SolanaStrategyEvent) -> Response<State> {
        match event {
            ...
            SolanaStrategyEvent::ForAgent(AgentEvent::Collect) => Transition(State::collecting(0)),
            SolanaStrategyEvent::ForAgent(AgentEvent::Buy(amt)) => {
                Transition(State::buying(amt.clone(), 0))
            }
            SolanaStrategyEvent::ForAgent(AgentEvent::Sell(amt)) => {
                Transition(State::selling(amt.clone(), 0))
            }
            _ => Super,
        }
    }
    ...
    async fn transfer(&mut self, batch: &Vec<SolanaTransferActionPayload>, retry: &i64) {
        ...
        let fee_payer = if batch
            .iter()
            .find(|x| x.amount == Amount::MaxAndClose && x.asset == Asset::Sol)
            .is_some()
        {
            self.main_wallet.clone().unwrap_or(self.agent_key.clone())
        } else {
        ...
        self.queue_action(SolanaAction::new_with_feepayer(
            self.agent_key.clone(),
            fee_payer,
            batch
                .iter()
                .map(|x| SolanaActionPayload::SolanaTransferActionPayload(x.clone()))
                .collect(),
        ))
        .await;
    }
    ...
    async fn transferring(
        ...
        self.transition_to_success_or_error(
            event,
            State::transferring(batch.clone(), retry + 1),
            retry,
        )
        .await
    }
    ...
    async fn collecting(&mut self, retry: &i64, event: &SolanaStrategyEvent) -> Response<State> {
    ...
    async fn buy_for_all(&mut self, retry: &i64) {
        ...
        debug!(
            "Agent `{:?}` buying tokens for all balance, retry: {retry}",
            self.pubkey(),
        );
    ...
    async fn sell(&mut self, amt: &Amount, retry: &i64) {
        ...
        .await;
        debug!(
            "Agent `{:?}` selling {:?} tokens action queued",
            self.pubkey(),
            amt
        );
    }
        ...
        match event {
            ...
            _ => {
                trace!(
                    ...
                    self.trader.strategy_instance_id,
                    self.trader.id
                );
            }
        }
